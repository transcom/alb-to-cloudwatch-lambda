...
const logStreamName = 'test' //Name of the log stream goes here;
...
exports.handler = (event, context, callback) => {
    ...
    function putLogEvents (sequenceToken, logData) {
        ...
        const fields = {
            ...
            "target_processing_time": null,
            "response_processing_time": null,
        ...
        rl.on('line', (line) => {
            ...
            const regex = /[^\s"]+|"([^"]*)"/gi;
            const logLine = line.match(regex);
            const logJson = Object.assign({}, fields);
            logLine.forEach((entry, index) => {
                logJson[Object.keys(logJson)[index]] = entry
            });
            batch.push({
                message: JSON.stringify(logJson),
                timestamp: tval
            });
        });
